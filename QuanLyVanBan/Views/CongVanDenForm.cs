using QuanLyVanBan.Authentications;
using QuanLyVanBan.Commons;
using QuanLyVanBan.Models;
using QuanLyVanBan.Repositories;
using System;
using System.Collections.Generic;
using System.Drawing;
using System.IO;
using System.Linq;
using System.Security.Principal;
using System.Threading;
using System.Windows.Forms;

namespace QuanLyVanBan.Views
{
	public partial class CongVanDenForm : Form
	{
		private readonly QuanLyVanBanDenRepository _repository = new QuanLyVanBanDenRepository();
		private readonly PhongBanRepository _phongBanRepository = new PhongBanRepository();

		public CongVanDenForm()
		{
			InitializeComponent();
			IPrincipal currentPrincipal = Thread.CurrentPrincipal;
			// Check if the current principal is of the custom principal type
			if (currentPrincipal is CustomPrincipal customPrincipal)
			{
				if (!customPrincipal.IsInRole("Cán bộ quản trị", "Cán bộ văn thư", "Ban giám hiệu"))
				{
					btnThem.Enabled = false;
					btnSua.Enabled = false;
					btnXoa.Enabled = false;
				}
			}
		}

		private void CongVanDenForm_Load(object sender, EventArgs e)
		{
			HienThi();
			LoadComboBox();
		}

		private void dtgv_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
		{
			foreach (DataGridViewRow row in dtgv.Rows)
			{
				if (dtgv.Columns[9].HeaderText == "Trạng thái" && row.Cells[9].Value.ToString() == "Đang xử lý")
				{
					row.DefaultCellStyle.ForeColor = Color.Red;
				}
			}
		}

		private void HienThi()
		{
			dtgv.DataSource = _repository.GetAll() ?? new List<QuanLyVanBanDen>();

			string[] headers = Constant.CongVanDenHeaders;
			int i = 0;
			foreach (var header in headers)
			{
				dtgv.Columns[i].HeaderText = header;
				i++;
			}
		}

		private void btnThoat_Click(object sender, EventArgs e)
		{
			this.Hide();
			var mainForm = new MainForm1();
			mainForm.ShowDialog();
		}

		private void btnThem_Click(object sender, EventArgs e)
		{
			try
			{
				if (!CheckNullOrWhiteSpace(txtSoDen.Text, txtSoKyHieuVB.Text, txtNoiGui.Text, txtNoiDung.Text, txtYKienChiDao.Text, txtTepDinhKem.Text, cbBoxDonVi.Text))
				{
					throw new Exception("Thông tin công văn không hợp lệ");
				}

				var exist = _repository.Get(txtSoKyHieuVB.Text, txtSoDen.Text);
				if (exist != null)
				{
					throw new Exception("Công văn này đã tồn tại");
				}

				var congVanDen = new QuanLyVanBanDen()
				{
					Soden = txtSoDen.Text.Trim(),
					SoVBden = txtSoKyHieuVB.Text.Trim(),
					NgayVBden = dtNgayVB.Value,
					NoiGui = txtNoiGui.Text.Trim(),
					TrichYeu = txtNoiDung.Text.Trim(),
					DoMat = cbBoxDoMat.SelectedValue.ToString(),
					YKienChiDao = txtYKienChiDao.Text.Trim(),
					DonViThucHien = cbBoxDonVi.Text.Trim(),
					TrangThaiVB = cbxTrangThai.SelectedValue.ToString(),
					GhiChu = txtGhiChu.Text.Trim()
				};

				congVanDen.TepDinhKem = SaveAttached(txtTepDinhKem.Text);
				_repository.Add(congVanDen);
				MessageBox.Show("Thêm thành công", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
				HienThi();
				Clear();
			}
			catch (Exception ex)
			{
				MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}

		private void btnSua_Click(object sender, EventArgs e)
		{
			try
			{
				if (!CheckNullOrWhiteSpace(txtSoDen.Text, txtSoKyHieuVB.Text, txtNoiGui.Text, txtNoiDung.Text, txtYKienChiDao.Text, txtTepDinhKem.Text, cbBoxDonVi.Text))
				{
					throw new Exception("Thông tin công văn không hợp lệ");
				}

				var exist = _repository.Get(txtSoKyHieuVB.Text, txtSoDen.Text);
				if (exist == null)
				{
					throw new Exception("Công văn này không tồn tại");
				}

				exist.NgayVBden = dtNgayVB.Value;
				exist.NoiGui = txtNoiGui.Text.Trim();
				exist.TrichYeu = txtNoiDung.Text.Trim();
				exist.DoMat = cbBoxDoMat.SelectedValue.ToString();
				exist.YKienChiDao = txtYKienChiDao.Text.Trim();
				exist.DonViThucHien = cbBoxDonVi.Text.Trim();
				exist.TrangThaiVB = cbxTrangThai.SelectedValue.ToString();
				exist.GhiChu = txtGhiChu.Text.Trim();
				exist.TepDinhKem = SaveAttached(txtTepDinhKem.Text);
				_repository.Update(exist);
				MessageBox.Show("Cập nhật thành công", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
				HienThi();
				Clear();
			}
			catch (Exception ex)
			{
				MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}

		private void btnXoa_Click(object sender, EventArgs e)
		{
			try
			{
				if (CheckNullOrWhiteSpace(txtSoKyHieuVB.Text, txtSoDen.Text))
				{
					var soKyHieuVB = txtSoKyHieuVB.Text;
					var soDen = txtSoDen.Text;
					var exist = _repository.Get(txtSoKyHieuVB.Text, txtSoDen.Text);
					if (exist == null)
					{
						throw new Exception("Công văn này không tồn tại");
					}
					DialogResult result = MessageBox.Show("Bạn có chắc chắn muốn xóa không?", "Xóa", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);

					if(result == DialogResult.Yes)
					{
						_repository.Delete(soKyHieuVB, soDen);
						DeleteAttachedFile(txtTepDinhKem.Text);
						MessageBox.Show("Xóa thành công", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
						HienThi();
						Clear();
					}
				}
				else
				{
					throw new Exception("Thông tin không hợp lệ");
				}
			}
			catch (Exception ex)
			{
				MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}

		private void btnReset_Click(object sender, EventArgs e)
		{
			Clear();
			HienThi();
		}

		private void btnTim_Click(object sender, EventArgs e)
		{
			try
			{
				var filter = txtTim.Text.Trim();
				if (!CheckNullOrWhiteSpace(filter))
				{
					throw new Exception("Dữ liệu bạn nhập không hợp lệ");
				}

				dtgv.DataSource = _repository.Find(filter);
			}
			catch (Exception ex)
			{
				MessageBox.Show(ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}

		private void btnChon_Click(object sender, EventArgs e)
		{
			OpenFileDialog fileDialog = new OpenFileDialog();
			fileDialog.Filter = "Text Files (*.txt)|*.txt|Doc Files (*.docx)|*.docx|XLS files (*.xls, *.xlt)|*.xls;*.xlt|XLSX files (*.xlsx, *.xlsm, *.xltx, *.xltm)|*.xlsx;*.xlsm;*.xltx;*.xltm|All files (*.*)|*.*";

			if (fileDialog.ShowDialog() == DialogResult.OK)
			{
				txtTepDinhKem.Text = fileDialog.FileName;
			}
		}

		private void dtgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
		{
			
		}

		private void dtgv_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex > -1)
			{
				var selectedRow = e.RowIndex;
				txtSoDen.Enabled = false;
				txtSoKyHieuVB.Enabled = false;

				txtSoDen.Text = dtgv.Rows[selectedRow].Cells[0].Value.ToString();
				txtSoKyHieuVB.Text = dtgv.Rows[selectedRow].Cells[1].Value.ToString();
				dtNgayVB.Value = (DateTime)dtgv.Rows[selectedRow].Cells[2].Value;
				txtNoiGui.Text = dtgv.Rows[selectedRow].Cells[3].Value.ToString();
				txtNoiDung.Text = dtgv.Rows[selectedRow].Cells[4].Value.ToString();
				var doMat = dtgv.Rows[selectedRow].Cells[5].Value.ToString();
				cbBoxDoMat.SelectedIndex = Constant.DoMat.IndexOf(doMat);
				txtTepDinhKem.Text = dtgv.Rows[selectedRow].Cells[6].Value.ToString();
				txtYKienChiDao.Text = dtgv.Rows[selectedRow].Cells[7].Value.ToString();
				cbBoxDonVi.Text = dtgv.Rows[selectedRow].Cells[8].Value.ToString();
				var trangThai = dtgv.Rows[selectedRow].Cells[9].Value.ToString();
				cbxTrangThai.SelectedIndex = trangThai.IndexOf(trangThai);
				txtGhiChu.Text = dtgv.Rows[selectedRow].Cells[10].Value.ToString();
			}
		}

		private string SaveAttached(string filePath)
		{
			string baseDirectory = AppDomain.CurrentDomain.BaseDirectory;
			string destinationFolder = Path.Combine(baseDirectory, "CongVanDen");

			// Ensure the "Attached" folder exists; create it if it doesn't.
			if (!Directory.Exists(destinationFolder))
			{
				Directory.CreateDirectory(destinationFolder);
			}

			var fileName = Path.GetFileNameWithoutExtension(filePath);
			var extension = Path.GetExtension(filePath);
			var path = Path.GetFullPath(filePath);
			var destinationFilePath = Path.Combine(destinationFolder, fileName + extension);
			if (!filePath.Contains(destinationFolder))
			{
				File.Copy(path, destinationFilePath, true);
			}
			return destinationFilePath;
		}

		private void DeleteAttachedFile(string filePath)
		{
			if (File.Exists(filePath))
			{
				File.Delete(filePath);
			}
		}

		private bool CheckNullOrWhiteSpace(params string[] value)
		{
			foreach (var item in value)
			{
				if (string.IsNullOrWhiteSpace(item))
				{
					return false;
				}
			}

			return true;
		}

		private void Clear()
		{
			txtSoDen.Enabled = true;
			txtSoKyHieuVB.Enabled = true;

			txtSoDen.Text = string.Empty;
			txtSoKyHieuVB.Text = string.Empty;
			dtNgayVB.Value = DateTime.Now;
			txtNoiGui.Text = string.Empty;
			txtNoiDung.Text = string.Empty;
			cbBoxDoMat.SelectedIndex = 0;
			txtTepDinhKem.Text = string.Empty;
			txtYKienChiDao.Text = string.Empty;
			cbBoxDonVi.SelectedIndex = 0;
			cbBoxDonVi.Text = string.Empty;
			cbxTrangThai.SelectedIndex = 0;
			txtGhiChu.Text = string.Empty;
			txtTim.Text = string.Empty;
			HienThi();
		}

		private void LoadComboBox()
		{
			cbBoxDoMat.DataSource = Constant.DoMat;
			cbxTrangThai.DataSource = Constant.TrangThai;

			var phongBans = _phongBanRepository.GetAll().ToList();
			var donViDataSource = new Dictionary<string, string>() { {"", ""} };
			foreach (var phongBan in phongBans)
			{
				donViDataSource.Add(phongBan.MaDV, phongBan.TenDV);
			}

			cbBoxDonVi.DataSource = new BindingSource(donViDataSource, null);
			cbBoxDonVi.DisplayMember = "Value";
			cbBoxDonVi.ValueMember = "Key";
			cbBoxDonVi.DropDownHeight = cbBoxDonVi.Font.Height * 10;
		}
	}
}
